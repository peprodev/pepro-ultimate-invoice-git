...
License: Pepro.dev License
License URI: https://pepro.dev/privacy/
*/
namespace peproulitmateinvoice;
use voku\CssToInlineStyles\CssToInlineStyles;

// TODO: check for shipping and billing address, if either was empty, replace it
// TODO: when disable wc and try to enable profile, debug err throws
// TODO: enable pretty links instead of $_GET methode
    ...
    class PeproUltimateInvoice
    {
        ...
        public function __construct()
        {
            ...
            */
            ...
            if ("yes" == $this->tpl->get_attach_pdf_invoices_to_mail()){ // attach pdf to woocommerce emails automaticlly
        ...
        * @return void
        */
        protected function init_tracker()
        {
        ...
        public function init_plugin()
        {
            ...
            // add_rewrite_rule( '^invoice-pdf/([^/]*)/?', 'index.php?invoice-pdf=$matches[1]', 'top');
            // add_rewrite_rule('^invoice/([^/]*)/?', 'index.php?invoice=$matches[1]', 'top');
            ...
            // add_action( "template_redirect", array( $this,"url_rewrite_templates"), 100000);
            ...
            add_filter( "woocommerce_admin_shipping_fields", function($ar){
                ...
                $ar['puiw_invoice_shipdatefa']  = array(
                  'label' => __( 'Shipping Date (Shamsi)', $this->td ),
                  'class'   => 'long persianDatepicker disabled',
                  'placeholder' => __( 'Select Shipping Date', $this->td ),
                  'custom_attributes' => array("readonly"=>"true"),
                );
                $ar['puiw_invoice_shipdate']  = array(
                  'label' => __( 'Shipping Date (Gregorian)', $this->td ),
                  'class'   => 'long persianDatepicker disabled',
                  'placeholder' => __( 'Select Shipping Date', $this->td ),
                  'custom_attributes' => array("readonly"=>"true"),
                );
                $ar['puiw_invoice_track_id']  = array(
                    'label' => __( 'Shipping Track ID', $this->td ),
                    'class'   => 'long',
                    'placeholder' => __( 'Enter Shipping Track ID', $this->td ),
        ...
        public function wc_save_shop_order_metabox($post_id)
        {
            ...
            if ( !isset( $_POST['puiw_shopmngr_provided_note'] ) ) {
        		...
        		// } else {
        		// 	update_post_meta( $post_id, 'puiw_billing_uin', sanitize_text_field( $_POST['puiw_billing_uin'] ));
        		// }
        		...
        		// if ( !isset( $_POST['puiw_invoice_track_id'] ) ) {
        		// 	delete_post_meta( $post_id, 'puiw_invoice_track_id' );
        		// } else {
        		// 	update_post_meta( $post_id, 'puiw_invoice_track_id', sanitize_text_field( $_POST['puiw_invoice_track_id'] ));
        		// }
        ...
        {
            // if ( ! empty( $_POST['puiw_shopmngr_provided_note'] ) ) {
            //     update_post_meta( $order_id, 'puiw_shopmngr_provided_note', sanitize_textarea_field($_POST['puiw_shopmngr_provided_note']));
            // }
                ...
                switch ($_POST['wparam']) {
                  ...
                  case "send-mail-html":
                    ...
                    if (!empty($email)){
                      ...
                      if ($wp_mail) {
                        wp_send_json_success( array("msg"=> sprintf(__("Sending email to <br><strong>%s</strong><br>was successfully done",$this->td), is_array($email) ? count($email) > 3 ? sprintf(__("%d emails",$this->td),count($email)) : implode("<br>", $email) : $email)));
                      }else{
                        wp_send_json_error( array("msg"=> sprintf(__("Error sending email to <br><strong>%s</strong>",$this->td), $email)));
                      }
                    }else{
