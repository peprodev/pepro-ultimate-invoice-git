    ...
    class PeproUltimateInvoice_Print
    {
        ...
        public function get_default_dynamic_params($order_id,$order)
        {
          $opts = array(
              ...
              "store_logo"                                => $this->fn->get_store_logo(),
          ...
          $opts["customer_fullname"]          = apply_filters("puiw_printinvoice_getcustomer_fullname",         ($use_billing == "billing") ? "{$order->get_billing_first_name()} {$order->get_shipping_last_name()}" : "{$order->get_shipping_first_name()} {$order->get_shipping_last_name()}", $opts, $order, $use_billing);
        ...
        public function create_html($order_id=0, $MODE="HTML",$part="",$email_printout="",$skipAuth=false)
        {
          ...
          if ("PDF" == $MODE) { $opts["invoice_final_prices"] = $this->get_order_final_prices_pdf($order); }
          $opt = apply_filters("puiw_printinvoice_create_html_options", $opts, $order);
          $opt["CURENT_DIR_URL"] = PEPROULTIMATEINVOICE_URL . "/template/{$opt["template"]}";
          ...
          if ("PDF_EXTRA_STYLE" == $MODE) { return $main_css_style_inline;}
          if ("PDF" == $MODE) {
              $template = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.pdf.tpl");
              if(!empty($part)){ $template = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.pdf.{$part}.tpl"); }
              $product_row_RAW = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.pdf.row.tpl");
          }
          else{
              $extrainvoiceheaddata   = '';
              $main_css_style         = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/style.css");
              if ($email_printout){   $main_css_style = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/style.email.css"); }
              ...
              $body_content           .= file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.tpl");
              $invoicehtmltitle       = "{{{invoice_title}}} | {{{store_name}}}";
              // $extrainvoiceheaddata   .= '<script src="'.PEPROULTIMATEINVOICE_URL.'/assets/js/JsBarcode.code128.min.js"></script>';
              // $extrainvoiceheaddata   .= '<script src="'.PEPROULTIMATEINVOICE_URL.'/assets/js/qrcode.min.js"></script>';
              // $extrainvoiceheaddata   .= '<script>window.onload = function(){ JsBarcode(".barcode").init(); new QRCode("invoice_qrcode", { text: "{{{invoice_qrcode}}}", width: 84, height: 84, colorDark : "#000000", colorLight : "#ffffff", correctLevel : QRCode.CorrectLevel.H }); };</script>';
              ...
              $product_row_RAW        = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.row.tpl");
              // if ($email_printout){$product_row_RAW .= $s;}
          ...
          foreach ( $order->get_items() as $item_id => $item ) {
              ...
              $tax = wc_price($item->get_subtotal_tax());
              $taxclass = $item->get_tax_class();
              $taxstat = $item->get_tax_status();
              $allmeta = $item->get_meta_data();
              $somemeta = $item->get_meta('_whatever', true);
              $description = $product->get_purchase_note();
              $type = $item->get_type();
              $weight = $this->fn->get_product_weight($item->get_product_id());
              $dimension = $this->fn->get_product_dimension($item->get_product_id());
              $percentage = round( ( ( (int)$product->get_regular_price() - (int)$product->get_sale_price() ) / (int)$product->get_regular_price() ) * 100 );
              $discount = wc_price( (int)$product->get_regular_price() - (int)$product->get_sale_price() );
              $sku = $product->get_sku();
              ...
              if ($product_weight) {
                ...
                $opt["invoice_total_weight"] = $this->fn->get_format_weight($total_weight);
              }
              ...
              $optm = array(
                ...
                "discount" => $discount,
                "weight" => $weight,
                "total_weight" => $this->fn->get_format_weight(floatval($product_weight*$quantity)),
                "dimension" => $dimension,
                "tax" => $tax,
                "nettotal" => $subtotal,
        ...
        public function create_pdf($order_id=0, $force_download = false, $MODE="I")
        {
          ...
          $template = $this->fn->get_template();
          $contents = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$template}/default.cfg");
          ...
          $mpdf = new \Mpdf\Mpdf(array(
            ...
            if (!$order) {
              ...
              // echo "<pre dir='ltr' style='border:1px solid gray'>". print_r([$stylesheet, $PDF_EXTRA_STYLE],1) ."</pre>";exit;
              ...
              $footerhtml = "<div class='footerauto' style='text-align:center;' dir='ltr'><strong>".
              sprintf(_x('%s -- Page {PAGENO} / {nbpg}', 'invoice-template', $PeproUltimateInvoice->td),$pdf_title)."</strong><br>".
              sprintf(_x('Created by %s at %s', 'invoice-template', $PeproUltimateInvoice->td),$PeproUltimateInvoice->title_t,$datec)."</div>";
        ...
        public function create_slips($order_id=0, $MODE="HTML")
        {
          ...
          $extrainvoiceheaddata = '';
          $main_css_style = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/style.slips.css");
          ...
          $body_content .= file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.slips.tpl");
          $invoicehtmltitle = "{{{invoice_title}}} | {{{store_name}}}";
          $extrainvoiceheaddata .= '<script src="'.PEPROULTIMATEINVOICE_URL.'/assets/js/JsBarcode.code128.min.js"></script>';
          $extrainvoiceheaddata .= '<script src="'.PEPROULTIMATEINVOICE_URL.'/assets/js/qrcode.min.js"></script>';
          $extrainvoiceheaddata .= '<script>window.onload = function(){ JsBarcode(".barcode").init(); };</script>';
        ...
        public function create_inventory($order_id=0, $MODE="HTML")
        {
          ...
          $extrainvoiceheaddata = '';
          $main_css_style = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/style.inventory.css");
          $body_content = '<p style="text-align:center;"><a class="print-button" href="javascript:;" onclick="window.print();return false;">'.__("PRINT",$this->td).'</a></p>';
          $body_content .= file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.inventory.tpl");
          $invoicehtmltitle = "{{{invoice_title}}} | {{{store_name}}}";
          $extrainvoiceheaddata .= '<script src="'.PEPROULTIMATEINVOICE_URL.'/assets/js/JsBarcode.code128.min.js"></script>';
          $extrainvoiceheaddata .= '<script src="'.PEPROULTIMATEINVOICE_URL.'/assets/js/qrcode.min.js"></script>';
          $extrainvoiceheaddata .= '<script>window.onload = function(){ JsBarcode(".barcode").init(); };</script>';
          ...
          $product_row_RAW = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$opt["template"]}/template.row.inventory.tpl");
          $n=0;$total_weight = 0;
          foreach ( $order->get_items() as $item_id => $item ) {
              $n+=1;
              $product_row = ($product_row_RAW);
        ...
        protected function CheckPDFRequirements($md = false)
        {
          $ok = true;
          foreach ( array(
              "mbstring" => "extension",
              ...
              if ($md){
                echo
                  ("extension" == $value)
                  ?
                  (extension_loaded($key) ? "<p>extension <strong>$key</strong>: OK</p>" : "<p>extension <strong>$key</strong>: DOES NOT EXISTS</p>" )
                  :
                  (function_exists($key) ? "<p>function <strong>$key</strong>: OK</p>" : "<p>function <strong>$key</strong>: DOES NOT EXISTS</p>")
                ;
              }else{
                if (!$ok){ return $ok;}
                if ("extension" == $value){
        ...
        {
          $template = $this->fn->get_template();
          $template = file_get_contents(PEPROULTIMATEINVOICE_DIR . "/template/{$template}/style.pdf.css");
          $opts = $this->get_default_dynamic_params($order_id,$order);
